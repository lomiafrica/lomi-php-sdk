...
```php
eventsEventIdGet($event_id): \LomiSDK\Model\Events
```
...
$apiInstance = new LomiSDK\Api\EventApi(
...
try {
    $result = $apiInstance->eventsEventIdGet($event_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling EventApi->eventsEventIdGet: ', $e->getMessage(), PHP_EOL;
}
```
...
```php
eventsGet($limit, $offset, $sort): \LomiSDK\Model\EventsGet200Response
```
...
try {
    $result = $apiInstance->eventsGet($limit, $offset, $sort);
    print_r($result);
} catch (Exception $e) {
...
[**\LomiSDK\Model\EventsGet200Response**](../Model/EventsGet200Response.md)
