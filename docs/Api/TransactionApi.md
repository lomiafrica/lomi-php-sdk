...
```php
transactionsGet($limit, $offset, $sort): \LomiSDK\Model\TransactionsGet200Response
```
...
$apiInstance = new LomiSDK\Api\TransactionApi(
...
try {
    $result = $apiInstance->transactionsGet($limit, $offset, $sort);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling TransactionApi->transactionsGet: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\LomiSDK\Model\TransactionsGet200Response**](../Model/TransactionsGet200Response.md)
...
```php
transactionsTransactionIdGet($transaction_id): \LomiSDK\Model\Transactions
```
...
$transaction_id = 'transaction_id_example'; // string | The transaction ID

try {
    $result = $apiInstance->transactionsTransactionIdGet($transaction_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling TransactionApi->transactionsTransactionIdGet: ', $e->getMessage(), PHP_EOL;
}
```
