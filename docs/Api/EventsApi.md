...
```php
listEvents($limit, $offset, $sort): \LomiSDK\Model\ListEvents200Response
```
...
$apiInstance = new LomiSDK\Api\EventsApi(
...
try {
    $result = $apiInstance->listEvents($limit, $offset, $sort);
    print_r($result);
} catch (Exception $e) {
...
[**\LomiSDK\Model\ListEvents200Response**](../Model/ListEvents200Response.md)
...
```php
retrieveEvent($event_id): \LomiSDK\Model\Events
```
...
$event_id = 'event_id_example'; // string | Unique identifier for the event

try {
    $result = $apiInstance->retrieveEvent($event_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling EventsApi->retrieveEvent: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\LomiSDK\Model\Events**](../Model/Events.md)
